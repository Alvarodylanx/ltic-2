...
const orderSteps = [
  { icon: Package,  title: { en: 'Browse Our Catalog',  fr: 'Parcourez Notre Catalogue' }, desc: { en: 'Explore industrial products — timber, generators, lubricants, and more.', fr: 'Explorez nos produits industriels — bois, générateurs, lubrifiants et plus encore.' }, action: { en: 'View Catalog',     fr: 'Voir le Catalogue' },    href: '/products' },
  { icon: FileText, title: { en: 'Request a Quote',     fr: 'Demandez un Devis'         }, desc: { en: 'Fill our quote form with product, quantity, destination. Takes under 2 minutes.', fr: 'Remplissez notre formulaire avec produit, quantité, destination. Moins de 2 minutes.' }, action: { en: 'Get a Quote', fr: 'Obtenir un Devis' },         href: '/quote' },
  { icon: Clock,    title: { en: 'Receive Your Offer',  fr: 'Recevez Votre Offre'       }, desc: { en: 'Custom price with freight costs, customs fees, and delivery timeline within 24–48 hours.', fr: 'Offre personnalisée avec frais de transport et délais sous 24–48h.' }, action: null, href: null },
  { icon: Truck,    title: { en: 'Track Your Shipment', fr: 'Suivez Votre Livraison'    }, desc: { en: 'We handle customs, freight, logistics. Track your order in real time.', fr: 'Nous gérons tout — douanes, fret, logistique. Suivez votre commande en temps réel.' }, action: { en: 'Track a Shipment', fr: 'Suivre une Livraison' }, href: '/tracking' },
];
...
const whyCards = [
  ...
  { icon: Globe2,     en: 'Global Network', fr: 'Réseau Mondial', descEn: 'Established connections across 30+ countries.',        descFr: 'Connexions établies dans 30+ pays.' },
  { icon: Zap,        en: 'Efficiency',     fr: 'Efficacité',     descEn: 'Optimized supply chains reducing cost and time.',      descFr: "Chaînes optimisées réduisant coûts et délais." },
  { icon: TrendingUp, en: 'Growth',         fr: 'Croissance',     descEn: 'Strategic partnerships opening new markets.',          descFr: "Partenariats ouvrant de nouveaux marchés." },
];
...
export default function HomePage() {
  ...
  return (
    <>
      {/* ═══════════════════════════════════════════════
          HERO — editorial headline + stats command panel
          ═══════════════════════════════════════════════ */}
      <section className="relative min-h-[92vh] sm:min-h-[90vh] bg-sidebar flex items-center overflow-hidden">
        {/* Background photo */}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-24 lg:py-28 lg:pl-12 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] xl:grid-cols-[1fr_440px] gap-10 lg:gap-16 items-center">
            ...
            <motion.div variants={stagger} initial="hidden" animate="show">
              ...
              <motion.p variants={fadeInUp}
                className="text-sidebar-foreground/65 text-base sm:text-lg leading-relaxed mb-8 max-w-xl">
                {L({
              ...
              <motion.div variants={fadeInUp} className="flex flex-wrap gap-3 sm:gap-4">
                <Button asChild size="lg" className="font-display font-semibold text-sm px-6 sm:px-8 rounded-sm shadow-none">
                  <Link href="/quote">
                ...
                <Button asChild size="lg" variant="outline"
                  className="font-display font-semibold text-sm px-6 sm:px-8 rounded-sm bg-transparent border-sidebar-foreground/30 text-sidebar-foreground hover:bg-sidebar-foreground/10 hover:text-sidebar-foreground hover:border-sidebar-foreground/50">
                  <Link href="/services">{L({ en: 'Our Services', fr: 'Nos Services' })}</Link>
                </Button>
              </motion.div>

              {/* Mobile stats — 2×2 grid, only visible below lg */}
              <motion.div
                variants={stagger}
                initial="hidden"
                animate="show"
                className="grid grid-cols-2 gap-2 mt-8 lg:hidden"
              >
                {stats.map((stat, i) => (
                  <motion.div key={i} variants={scaleIn}
                    className="border border-primary/25 bg-sidebar-foreground/5 p-3 sm:p-4">
                    <span className="stat-num font-display font-bold text-2xl sm:text-3xl text-primary block mb-1">
                      {stat.value}
                    </span>
                    <span className="text-sidebar-foreground/50 text-[10px] sm:text-xs uppercase tracking-wide font-medium leading-tight">
                      {L(stat)}
                    </span>
                  </motion.div>
                ))}
            ...
            {/* Right: Stats command panel — desktop only */}
      ...
      {/* ═══════════════════════════════════
          ...
          ═══════════════════════════════════ */}
      <section className="bg-foreground py-10 sm:py-14 border-b border-sidebar-border/30">
          ...
          >
            {stats.map((stat, i) => (
              <motion.div key={i} variants={scaleIn} className="flex flex-col items-center">
                <span className="stat-num font-display font-bold text-3xl sm:text-4xl md:text-5xl text-primary mb-1.5 sm:mb-2">
                  {stat.value}
                </span>
                <span className="text-sidebar-foreground/55 text-[10px] sm:text-xs uppercase tracking-widest font-medium">
                  {L(stat)}
                </span>
              </motion.div>
            ))}
      ...
      {/* ═══════════════════════════════════
          PARTNERS MARQUEE
          ═══════════════════════════════════ */}
      <section className="relative bg-background py-12 sm:py-16 overflow-hidden border-b border-border">
        <motion.div
          variants={fadeInUp}
          initial="hidden"
          whileInView="show"
        ...
        <div className="marquee-wrap space-y-3 select-none">
          <div className="flex w-max marquee-left">
            {[...brandsRow1, ...brandsRow1].map((b, i) => (
              <div key={i}
                className="flex items-center gap-3 mx-2.5 px-4 py-2.5 bg-card border border-border rounded-sm hover:border-primary/40 transition-all duration-300 cursor-default flex-shrink-0">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-sm bg-muted border border-border flex items-center justify-center flex-shrink-0 overflow-hidden">
                  {b.logoUrl
                ...
                <div>
                  <p className="font-display font-bold text-xs sm:text-sm text-foreground leading-none">{b.name}</p>
                  <p className="text-[10px] sm:text-xs text-primary mt-0.5">{L({ en: b.sectorEn, fr: b.sectorFr })}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ═══════════════════════════════════
          SERVICES — Bento grid
          ═══════════════════════════════════ */}
      <section className="bg-card py-16 sm:py-20 lg:py-24 border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            variants={fadeInUp}
            initial="hidden"
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6">
              <h2 className="font-display font-bold text-2xl sm:text-3xl lg:text-4xl tracking-tight text-foreground">
                {L({ en: 'Comprehensive Business Solutions', fr: "Solutions d'Affaires Complètes" })}
              </h2>
              <Button asChild variant="outline" className="rounded-sm font-display font-semibold text-sm flex-shrink-0 self-start sm:self-auto w-fit">
                <Link href="/services">
          ...
          {/* Bento: featured 2×2 + 5 small cards */}
          <motion.div
            variants={stagger}
            initial="hidden"
            whileInView="show"
          ...
          >
            {/* FEATURED — full width on mobile, 2×2 on md+ */}
            <motion.div
              variants={fadeIn}
              className="relative sm:col-span-2 md:col-span-2 md:row-span-2 min-h-[260px] sm:min-h-[300px] md:min-h-0 overflow-hidden group cursor-default"
            >
              <Image
              ...
              <div className="absolute inset-0 p-6 sm:p-8 lg:p-10 flex flex-col justify-end">
                <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-sm bg-primary/20 border border-primary/40 flex items-center justify-center mb-4">
                  <Ship className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />
                </div>
                <p className="text-primary font-display font-semibold text-xs uppercase tracking-[0.18em] mb-1.5">
                  {L({ en: 'Core Service', fr: 'Service Principal' })}
                </p>
                <h3 className="font-display font-bold text-xl sm:text-2xl lg:text-3xl text-sidebar-foreground tracking-tight mb-2 sm:mb-3">
                  {L({ en: services[0].en, fr: services[0].fr })}
                </h3>
                <p className="text-sidebar-foreground/65 text-sm leading-relaxed max-w-lg hidden sm:block">
            ...
            {services.slice(1).map(({ icon: Icon, ...svc }) => (
              <motion.div
                key={svc.en}
                variants={fadeInUp}
                className="bento-card card-elevated bg-card border border-border p-5 sm:p-6 cursor-default group min-h-[130px] sm:min-h-[150px] md:min-h-0"
              >
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-sm bg-foreground flex items-center justify-center mb-3 sm:mb-4">
                  <Icon className="h-4 w-4 text-primary" />
                </div>
                <h3 className="font-display font-bold text-sm mb-1.5 group-hover:text-primary transition-colors duration-200">
      ...
      {/* ═══════════════════════════════════════════════
          WHY LTIC — 50/50 navy + white split
          ═══════════════════════════════════════════════ */}
      <section className="overflow-hidden border-b border-border">
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="lg:flex-1 bg-sidebar relative">
            ...
            <div className="relative px-5 sm:px-10 lg:px-16 xl:px-20 py-14 sm:py-20 lg:py-28 lg:max-w-2xl lg:ml-auto">
              <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={viewportOnce}>
                ...
                <motion.h2 variants={fadeInUp}
                  className="font-display font-bold text-2xl sm:text-3xl lg:text-4xl tracking-tight text-sidebar-foreground mb-5 sm:mb-6">
                  {L({ en: 'Your Strategic Partner for Global Operations', fr: 'Votre Partenaire Stratégique pour les Opérations Mondiales' })}
                </motion.h2>
                <motion.p variants={fadeInUp}
                  className="text-sidebar-foreground/60 mb-6 sm:mb-8 leading-relaxed text-sm sm:text-base">
                  {L({
          ...
          {/* RIGHT: White panel */}
          <div className="lg:flex-1 bg-background px-5 sm:px-10 lg:px-16 xl:px-20 py-14 sm:py-20 lg:py-28">
            <div className="lg:max-w-2xl">
              ...
              >
                {whyCards.map(({ icon: Icon, ...card }) => (
                  <motion.div key={card.en} variants={scaleIn}
                    className="bg-card border border-border p-4 sm:p-5 lg:p-6 card-elevated cursor-default">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-sm bg-foreground flex items-center justify-center mb-3 sm:mb-4">
                      <Icon className="h-4 w-4 text-primary" />
                    </div>
                    <h4 className="font-display font-bold text-xs sm:text-sm mb-1">{L({ en: card.en, fr: card.fr })}</h4>
                    <p className="text-muted-foreground text-xs leading-relaxed hidden sm:block">{L({ en: card.descEn, fr: card.descFr })}</p>
                  </motion.div>
                ))}
              </motion.div>
              ...
              <motion.div
                ...
                whileInView="show"
                viewport={viewportOnce}
                className="mt-4 p-4 sm:p-6 bg-foreground"
              >
                <p className="text-sidebar-foreground/45 text-xs uppercase tracking-widest font-display font-semibold mb-3 sm:mb-4">
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-4">
                  {[
                    ...
                    <div key={s.en} className="text-center">
                      <span className="stat-num font-display font-bold text-xl sm:text-2xl text-primary block">{s.val}</span>
                      <span className="text-sidebar-foreground/45 text-[10px] sm:text-xs uppercase tracking-wide mt-0.5 block">{L(s)}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
      ...
      {/* ═══════════════════════════════════
          FEATURED PRODUCTS
          ═══════════════════════════════════ */}
      <section className="bg-muted/50 py-16 sm:py-20 lg:py-24 border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            variants={fadeInUp}
            initial="hidden"
                    ...
                    >
                      ...
                      <div className="p-3 sm:p-4">
                        {product.categoryName && (
                          <span className="inline-block bg-primary/15 text-primary text-[10px] sm:text-xs rounded-sm px-1.5 py-0.5 mb-1.5 font-medium">
                            {product.categoryName}
                          </span>
                        )}
                        <h3 className="font-display font-semibold text-xs sm:text-sm leading-tight group-hover:text-primary transition-colors duration-200">
      ...
      <section className="bg-background py-16 sm:py-20 lg:py-24 border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <h2 className="font-display font-bold text-2xl sm:text-3xl lg:text-4xl tracking-tight mb-3 sm:mb-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
            ...
            {/* LEFT: Vertical route-line timeline */}
            <div className="relative">
              {/* The dashed route line */}
              <div className="absolute left-5 sm:left-6 top-8 bottom-8 route-line-dashed" />
              ...
              {orderSteps.map((step, i) => (
                <motion.div
                  ...
                  className="flex gap-3 sm:gap-5 mb-4 sm:mb-5 last:mb-0"
                >
                  {/* Waypoint circle */}
                  <div className="relative z-10 w-10 h-10 sm:w-12 sm:h-12 rounded-full border-2 border-primary bg-background flex items-center justify-center flex-shrink-0 shadow-[0_0_0_4px_hsl(var(--background))]">
                    <span className="font-display font-bold text-xs sm:text-sm text-primary">0{i + 1}</span>
                  </div>
                  {/* Step card */}
                  <div className="flex-1 bg-card border border-border p-4 sm:p-5 card-elevated">
                    <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
                      <step.icon className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-primary flex-shrink-0" />
                      <h3 className="font-display font-bold text-xs sm:text-sm">{L(step.title)}</h3>
                    </div>
                    <p className="text-muted-foreground text-xs leading-relaxed mb-2 sm:mb-3">{L(step.desc)}</p>
            ...
            <motion.div
              ...
              whileInView="show"
              viewport={viewportOnce}
              className="bg-sidebar rounded-sm p-6 sm:p-8 lg:p-10 relative overflow-hidden"
            >
              ...
              <div className="relative">
                ...
                <h3 className="font-display font-bold text-lg sm:text-xl lg:text-2xl text-sidebar-foreground tracking-tight mb-5 sm:mb-6">
                  {L({ en: 'Global Coverage Across All Sectors', fr: 'Couverture Mondiale Tous Secteurs' })}
                </h3>
                <div className="flex flex-wrap gap-2 mb-6 sm:mb-8">
                  {industries.map((ind) => (
                    <span key={ind.en}
                      className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-sidebar-border/50 border border-sidebar-border text-sidebar-foreground/70 text-xs font-medium rounded-sm">
                      {L(ind)}
                    </span>
                  ))}
                </div>
                <div className="flex flex-col gap-2.5 sm:gap-3">
                  <Button asChild size="sm" className="font-display font-semibold text-xs rounded-sm justify-start h-10">
                    <Link href="/products">
                  ...
                  <Button asChild size="sm" variant="outline"
                    className="font-display font-semibold text-xs rounded-sm justify-start bg-transparent border-sidebar-foreground/20 text-sidebar-foreground/70 hover:bg-sidebar-foreground/10 hover:text-sidebar-foreground hover:border-sidebar-foreground/40 h-10">
                    <Link href="/contact">
      ...
      {/* ═══════════════════════════════════════
          CTA — Centered dramatic navy
          ═══════════════════════════════════════ */}
      <section className="bg-sidebar relative overflow-hidden py-20 sm:py-24 lg:py-28">
        <div className="absolute inset-0 dot-grid opacity-55 pointer-events-none" />
        {/* Corner glows — hidden on mobile for performance */}
        <div className="absolute -top-40 -right-40 w-[500px] h-[500px] rounded-full bg-primary/8 blur-[120px] pointer-events-none hidden sm:block" />
        <div className="absolute -bottom-40 -left-40 w-[500px] h-[500px] rounded-full bg-primary/5 blur-[120px] pointer-events-none hidden sm:block" />
        <div className="absolute top-0 left-0 right-0 h-0.5 bg-primary" />

        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={viewportOnce}>
            <motion.p variants={fadeInUp}
              className="text-primary font-display font-semibold text-xs uppercase tracking-[0.2em] mb-5 sm:mb-6">
              {L({ en: 'Ready to Start?', fr: 'Prêt à Commencer ?' })}
            </motion.p>
            <motion.h2 variants={fadeInUp}
              className="text-hero font-display font-bold text-sidebar-foreground tracking-tight mb-5 sm:mb-6">
              {L({ en: 'Optimize Your Global Operations Today.', fr: "Optimisez Vos Opérations Mondiales Aujourd'hui." })}
            </motion.h2>
            <motion.p variants={fadeInUp}
              className="text-sidebar-foreground/55 text-base sm:text-lg max-w-xl mx-auto mb-8 sm:mb-10 leading-relaxed">
              {L({
            ...
            <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
              <Button asChild size="lg" className="font-display font-semibold text-sm px-8 rounded-sm shadow-none w-full sm:w-auto">
                <Link href="/contact">
              ...
              <Button asChild size="lg" variant="outline"
                className="font-display font-semibold text-sm px-8 rounded-sm bg-transparent border-sidebar-foreground/25 text-sidebar-foreground hover:bg-sidebar-foreground/10 hover:border-sidebar-foreground/40 hover:text-sidebar-foreground w-full sm:w-auto">
