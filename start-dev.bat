...
start "LTIC API — Port 4000" cmd /k "cd /d %~dp0 && pnpm --filter @ltic/api dev"

echo [4/4] Starting Web server (port 3000 + Turbopack)...
start "LTIC Web — Port 3000" cmd /k "cd /d %~dp0 && pnpm --filter @ltic/web dev"

echo.
echo Both servers are starting. Wait ~10 seconds then open:
